...
public:
   ...
   * \return 返回成功输出的字符长度，失败返回-1。
   */
   int putText(cv::Mat &img, std::vector<std::string> text, cv::Point &pos, cv::Scalar color);
...
private:
   ...
   int putWChar(cv::Mat &img, wchar_t wc, cv::Point &pos, cv::Scalar color);
